...
export const getArtists = async <T>(query: string, limit = limitPerPage, offset = 0): Promise<T> => {
...
export const getArtistAlbumsByQuery =
  async <T>(query: string, limit = limitPerPage, offset = 0): Promise<T> => {
    const response = await api.get(`/search`, {
      params: {
        q: query,
        type: "album",
